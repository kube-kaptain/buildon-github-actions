...
#
# Kaptain metadata sites: this library is one of three places that emit
# `kaptain.org/*` labels and annotations onto Kubernetes resources or related
# documents. When changing the key set or types here, also review the others
# so they stay consistent. See AnnotationLabelConsistency.md for the full set.
#
#   1. src/scripts/lib/kubernetes-metadata.bash::generate_metadata (this file)
#      Per-generated-resource. Full superset of keys.
#   2. src/scripts/main/layer-package-prepare (yq metadata block)
#      Per-layer KaptainPM.yaml. Subset of (1) minus kaptain.org/environment
#      (a layer is environment-agnostic; environment is bound at deploy).
...
#
# shellcheck disable=SC2154 # app_name, version_token, environment_token, product_token, build_timestamp, script_name, project_name_token, default_container, checksum_named set by caller
...
generate_yaml_map() {
  ...
  for pair in ${pairs}; do
    key="${pair%%=*}"
    value="${pair#*=}"
    # Quote every value so YAML always parses it as a string: cron expressions
    # start with '*', "false"/"0" would become bool/int, and ':' or '#' would
    # break the parse. Escape backslashes first, then double quotes - the only
    # two characters a YAML double-quoted scalar requires escaping.
    value="${value//\\/\\\\}"
    value="${value//\"/\\\"}"
...
#
# Reads from env (optional - keys are omitted when env var is empty/unset):
#   REPOSITORY_OWNER  - kaptain.org/owner label
#   BUILD_PLATFORM    - kaptain.org/built-by annotation
#   SOURCE_REPO       - kaptain.org/source-repository annotation
#   IMAGE_URI         - kaptain.org/image-uri annotation
#
# Arguments:
...
generate_metadata() {
  ...
  # Checksum-named resources must be byte-stable per content: their name is a
  # content checksum, so per-build/per-release metadata would churn the
  # checksum (accumulating dead resources and rippling renames) without a
  # real content change. Callers producing a checksum-marker name set
  # checksum_named=true; the volatile keys (build-timestamp, version labels,
  # image-uri) are then omitted. built-by is deliberately KEPT: a platform
  # change starts a new checksum series on purpose. See
  # todo-checked/PreEnvFixes.md.
  local volatile_metadata="true"
  if [[ "${checksum_named:-false}" == "true" ]]; then
    volatile_metadata="false"
  fi
  ...
  case "${type}" in
    labels)
      builtin="app=${app_name},app.kubernetes.io/name=${app_name}"
      if [[ "${volatile_metadata}" == "true" ]]; then
        builtin="${builtin},app.kubernetes.io/version=${version_token}"
      fi
      builtin="${builtin},app.kubernetes.io/managed-by=Kaptain,app.kubernetes.io/part-of=${product_token}"
      builtin="${builtin},kaptain.org/environment=${environment_token},kaptain.org/product=${product_token},kaptain.org/project-name=${project_name_token}"
      if [[ "${volatile_metadata}" == "true" ]]; then
        builtin="${builtin},kaptain.org/version=${version_token}"
      fi
    ...
    annotations)
      ...
      if [[ -n "${default_container}" ]]; then
        builtin="kubectl.kubernetes.io/default-container=${default_container},"
      fi
      if [[ "${volatile_metadata}" == "true" ]]; then
        builtin="${builtin}kaptain.org/build-timestamp=${build_timestamp},"
      fi
      builtin="${builtin}kaptain.org/generated-by=Generated by Kaptain ${script_name}"
      if [[ -n "${BUILD_PLATFORM:-}" ]]; then
        builtin="${builtin},kaptain.org/built-by=${BUILD_PLATFORM}"
      fi
      if [[ -n "${SOURCE_REPO:-}" ]]; then
        builtin="${builtin},kaptain.org/source-repository=${SOURCE_REPO}"
      fi
      if [[ -n "${IMAGE_URI:-}" ]] && [[ "${volatile_metadata}" == "true" ]]; then
        builtin="${builtin},kaptain.org/image-uri=${IMAGE_URI}"
      fi
