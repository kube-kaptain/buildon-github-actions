...
#
# Tests for hook-pre-tagging-tests script
...
setup() {
  TEST_DIR=$(create_test_dir "hook-pre-tagging")
...
@test "hook-pre-tagging-tests exports ALL variables (self-validating)" {
  # Extract exports directly from the hook script - test fails if script adds new exports
  local exports
  exports=$(extract_hook_exports "$SCRIPTS_DIR/hook-pre-tagging-tests")
...
@test "hook-pre-tagging-tests exports all inputs from sourced defaults (self-validating)" {
  ...
  run verify_hook_exports_all_inputs "$SCRIPTS_DIR/hook-pre-tagging-tests"
  if [[ "$status" -ne 0 ]]; then
    echo "$output" >&3
    return 1
  fi
