...
#
# Tests for hook-post-package-tests script
...
setup() {
  TEST_DIR=$(create_test_dir "hook-post-package-tests")
  ...
  export MANIFESTS_ZIP_SUB_PATH="target"
  export MANIFESTS_ZIP_FILE_NAME="my-image-manifests-1.2.3.zip"

  # Set required workflow inputs
  ...
  export BUILD_SCRIPTS_REPO_ROOT="/test/build-scripts-repo-root"
...
@test "hook-post-package-tests exports ALL variables (self-validating)" {
  # Extract exports directly from the hook script - test fails if script adds new exports
  local exports
  exports=$(extract_hook_exports "$SCRIPTS_DIR/hook-post-package-tests")

  # Generate a hook that dumps all expected exports
  generate_export_dump_hook "$HOOK_SCRIPT" "$HOOK_OUTPUT" $exports

  export HOOK_POST_PACKAGE_TESTS_SCRIPT_SUB_PATH="${HOOK_SCRIPT}"
  export HOOK_OUTPUT
...
@test "hook-post-package-tests skips when no hook script configured" {
  unset HOOK_POST_PACKAGE_TESTS_SCRIPT_SUB_PATH
...
@test "hook-post-package-tests exports all inputs from sourced defaults (self-validating)" {
  ...
  run verify_hook_exports_all_inputs "$SCRIPTS_DIR/hook-post-package-tests"
  if [[ "$status" -ne 0 ]]; then
    echo "$output" >&3
    return 1
  fi
...
@test "hook-post-package-tests DOCKER_IMAGE_FULL_URI aliases DOCKER_TARGET_IMAGE_FULL_URI" {
  cat > "${HOOK_SCRIPT}" << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
{
...
@test "hook-post-package-tests exports manifest package outputs" {
  cat > "${HOOK_SCRIPT}" << 'EOF'
#!/usr/bin/env bash
set -euo pipefail
{
  echo "MANIFESTS_SUBSTITUTED_SUB_PATH=${MANIFESTS_SUBSTITUTED_SUB_PATH}"
  echo "MANIFESTS_ZIP_SUB_PATH=${MANIFESTS_ZIP_SUB_PATH}"
  echo "MANIFESTS_ZIP_FILE_NAME=${MANIFESTS_ZIP_FILE_NAME}"
} > "${HOOK_OUTPUT}"
EOF
  chmod +x "${HOOK_SCRIPT}"
  ...
  [ "$status" -eq 0 ]
  [ -f "${HOOK_OUTPUT}" ]
  grep -q "MANIFESTS_SUBSTITUTED_SUB_PATH=target/manifests" "${HOOK_OUTPUT}"
  grep -q "MANIFESTS_ZIP_SUB_PATH=target" "${HOOK_OUTPUT}"
  grep -q "MANIFESTS_ZIP_FILE_NAME=my-image-manifests-1.2.3.zip" "${HOOK_OUTPUT}"
}
