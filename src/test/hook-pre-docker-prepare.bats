...
#
# Tests for hook-pre-docker-prepare script
...
setup() {
  TEST_DIR=$(create_test_dir "hook-pre-docker-prepare")
...
@test "hook-pre-docker-prepare exports ALL variables (self-validating)" {
  # Extract exports directly from the hook script - test fails if script adds new exports
  local exports
  exports=$(extract_hook_exports "$SCRIPTS_DIR/hook-pre-docker-prepare")
...
@test "hook-pre-docker-prepare exports all inputs from sourced defaults (self-validating)" {
  ...
  run verify_hook_exports_all_inputs "$SCRIPTS_DIR/hook-pre-docker-prepare"
  if [[ "$status" -ne 0 ]]; then
    echo "$output" >&3
    return 1
  fi
