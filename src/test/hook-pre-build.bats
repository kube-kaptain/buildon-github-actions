...
#
# Tests for hook-pre-build script
# This hook runs BEFORE everything - no step outputs available
# Verifies that ALL exported workflow input variables are accessible

load helpers

setup() {
  TEST_DIR=$(create_test_dir "hook-pre-build")
...
@test "hook-pre-build exports ALL variables (self-validating)" {
  local exports
  exports=$(extract_hook_exports "$SCRIPTS_DIR/hook-pre-build")

  generate_export_dump_hook "$HOOK_SCRIPT" "$HOOK_OUTPUT" $exports

  export HOOK_PRE_BUILD_SCRIPT_SUB_PATH="${HOOK_SCRIPT}"
  export HOOK_OUTPUT

  run "$SCRIPTS_DIR/hook-pre-build"
  [ "$status" -eq 0 ]
  [ -f "${HOOK_OUTPUT}" ]
...
@test "hook-pre-build exports all inputs from sourced defaults (self-validating)" {
  run verify_hook_exports_all_inputs "$SCRIPTS_DIR/hook-pre-build"
  if [[ "$status" -ne 0 ]]; then
    echo "$output" >&3
    return 1
  fi
...
@test "hook-pre-build skips when no hook script configured" {
  unset HOOK_PRE_BUILD_SCRIPT_SUB_PATH
