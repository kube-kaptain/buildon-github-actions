...
@test "generates Secret template with single file" {
  ...
  [[ "$manifest" == *"kind: Secret"* ]] || return 1
  [[ "$manifest" == *"type: Opaque"* ]] || return 1
  [[ "$manifest" == *"stringData:"* ]] || return 1
  [[ "$manifest" == *"database-password:"* ]] || return 1
  [[ "$manifest" == *'${DatabasePassword}'* ]] || return 1
}
...
@test "includes kaptain annotations" {
  ...
  [[ "$manifest" == *'kaptain.org/generated-by: "Generated by Kaptain generate-kubernetes-secret-template"'* ]] || return 1
...
@test "uses checksum injection by default" {
  ...
  manifest=$(read_manifest)
  [[ "$manifest" == *'name: ${ProjectName}-secret-checksum'* ]] || return 1
}
...
@test "uses plain name when checksum injection disabled" {
  ...
  [[ "$manifest" != *"secret-checksum"* ]] || return 1
}
...
@test "respects UPPER_SNAKE token name style" {
  ...
  manifest=$(read_manifest)
  [[ "$manifest" == *'${PROJECT_NAME}'* ]] || return 1
  [[ "$manifest" == *'${ENVIRONMENT}'* ]] || return 1
}
...
@test "handles content with special characters" {
  ...
  manifest=$(read_manifest)
  [[ "$manifest" == *'postgres://${DbUser}:${DbPassword}@host:5432/db?ssl=true'* ]] || return 1
}
...
@test "adds secret-template-specific additional labels" {
  ...
  manifest=$(read_manifest)
  [[ "$manifest" == *'secret-type: "credentials"'* ]] || return 1
}
...
@test "secret-template labels override global labels" {
  ...
  manifest=$(read_manifest)
  [[ "$manifest" == *'team: "secret-team"'* ]] || return 1
...
@test "suffix affects metadata.name with checksum" {
  ...
  [[ "$manifest" == *'name: ${ProjectName}-db-secret-checksum'* ]] || return 1
}
...
@test "suffix appends to custom sub-path for source directory" {
  ...
  manifest=$(cat "$OUTPUT_SUB_PATH/manifests/combined/secret-db.template.yaml")
  [[ "$manifest" == *"password:"* ]] || return 1
  [[ "$manifest" == *'${DbPassword}'* ]] || return 1
}
...
@test "custom sub-path without suffix uses path with static suffix" {
  ...
  [[ "$manifest" == *"secret-key:"* ]] || return 1
  [[ "$manifest" == *'${CustomSecret}'* ]] || return 1
}
...
@test "suffix with custom sub-path affects name and filename" {
  ...
  [[ "$manifest" == *'name: ${ProjectName}-api-secret-checksum'* ]] || return 1
}
...
@test "combined sub-path affects metadata.name with checksum" {
  ...
  [[ "$manifest" == *'name: ${ProjectName}-omg-secret-checksum'* ]] || return 1
}
...
@test "combined sub-path with suffix: name is ProjectName-combined-suffix-checksum" {
  ...
  [[ "$manifest" == *'name: ${ProjectName}-omg-wtf-secret-checksum'* ]] || return 1
}
...
# =============================================================================
# Checksum-named resources omit volatile metadata
# =============================================================================

@test "checksum-named Secret template omits volatile metadata, keeps stable identity" {
  create_secret_file "database-password" '${DatabasePassword}'
  ...
  manifest=$(read_manifest)
  # Volatile keys must be absent: they churn the secret checksum per build
  [[ "$manifest" != *"kaptain.org/build-timestamp:"* ]] || return 1
  [[ "$manifest" != *"app.kubernetes.io/version:"* ]] || return 1
  [[ "$manifest" != *"kaptain.org/version:"* ]] || return 1
  [[ "$manifest" != *"kaptain.org/image-uri:"* ]] || return 1
  # Stable identity stays; built-by deliberately kept (new series per platform)
  [[ "$manifest" == *'app.kubernetes.io/managed-by: "Kaptain"'* ]] || return 1
  [[ "$manifest" == *'kaptain.org/project-name: "${ProjectName}"'* ]] || return 1
  [[ "$manifest" == *'kaptain.org/generated-by: "Generated by Kaptain generate-kubernetes-secret-template"'* ]] || return 1
  [[ "$manifest" == *'kaptain.org/built-by: "test"'* ]] || return 1
