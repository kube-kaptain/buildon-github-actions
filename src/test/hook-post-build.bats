...
#
# Tests for hook-post-build script
# This hook runs AFTER everything - version outputs available
# Verifies that ALL exported workflow input variables are accessible

load helpers

setup() {
  TEST_DIR=$(create_test_dir "hook-post-build")
...
@test "hook-post-build exports ALL variables (self-validating)" {
  local exports
  exports=$(extract_hook_exports "$SCRIPTS_DIR/hook-post-build")

  generate_export_dump_hook "$HOOK_SCRIPT" "$HOOK_OUTPUT" $exports

  export HOOK_POST_BUILD_SCRIPT_SUB_PATH="${HOOK_SCRIPT}"
  export HOOK_OUTPUT

  run "$SCRIPTS_DIR/hook-post-build"
  [ "$status" -eq 0 ]
  [ -f "${HOOK_OUTPUT}" ]
...
@test "hook-post-build exports all inputs from sourced defaults (self-validating)" {
  run verify_hook_exports_all_inputs "$SCRIPTS_DIR/hook-post-build"
  if [[ "$status" -ne 0 ]]; then
    echo "$output" >&3
    return 1
  fi
...
@test "hook-post-build skips when no hook script configured" {
  unset HOOK_POST_BUILD_SCRIPT_SUB_PATH
